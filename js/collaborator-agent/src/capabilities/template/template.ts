...
export class TemplateCapability extends BaseCapability {
  ...
  createPrompt(context: MessageContext): ChatPrompt {
    ...
    const prompt = new ChatPrompt({
    ...
    }).function('process_template_request', 'Process a template-specific request', TEMPLATE_FUNCTION_SCHEMA,
      async (args: TemplateFunctionArgs) => {
        ...
        const response = `Template capability received: "${userRequest}" from user ${userName} in ${isPersonalChat ? 'personal' : 'group'} chat. Args: ${JSON.stringify(args)}`;
